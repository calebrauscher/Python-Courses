...
def build_rating_list(name_to_rating, names_final):
    ...
    return [[name_to_rating.get(name), name] for name in names_final]
...
def filter_by_cuisine(names_matching_price, cuisine_to_names, cuisines_list):
    ...
    for cuisine in cuisines_list:
        for restaurant in cuisine_to_names.get(cuisine):
...
def read_restaurants(file):
    ...
    with open(file, 'r') as fh:
        content = [line.strip() for line in fh.readlines()
                   if line.strip() != '']

    for n in range(0, len(content), 4):
        restaurant, rating, price, cuisine = content[n:n+4]
        ...
        for item in cuisine.split(','):
            if item in cuisine_to_names:
                ...
                cuisine_to_names[item] = [restaurant]
